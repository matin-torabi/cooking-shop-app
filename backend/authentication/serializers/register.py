# NOTE this is for registeration serializer
'''
I'm planning to implement two-step verification and 
I'm also using redis to generate one-time-password codes
because it's much faster than anything else.
'''
...
class FirstStepRegisterSerializer(serializers.Serializer):
    '''
    Parameters:
    ...
    def validate(self ,attrs , *args, **kwargs):
        ...
        if User.objects.filter(username=phone_number).exists():
            raise ValidationError({'message':"این شماره قبلا ثبت شده است"})
    ...
    def create(self, validated_data):
        ...
        # save temp_token and otp in cache
        cache.set(
            f"otp:{temp_token}",
...
class SecondStepRegisterSerializer(serializers.Serializer):
    """
    ...
    def create(self, validated_data):
        ...
        move_user_to_group(username=user.username , group_name='authenticated')
